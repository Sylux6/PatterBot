...
class Emoji(cmd.Cog):
    def __init__(self, bot):
        self.bot = bot
        self.emoji_path = "{}/emojis/".format(Path(__file__).parent.parent.parent.absolute())
        ...
        **[name] [file] [roles]** : adds an emoji to the server, if roles, restricts it for precised roles.
        ...
        if None in roles:
            ...
            try:
                ...
                embed = Embed(title= "Successfully created {} emoji.".format(str(emoji)),
                              color= COLOR,
    ...
    async def emoji_list(self, ctx:cmd.Context):
        ...
        for emoji in emojis:
            ...
            if emoji.managed:
                ...
                message += " - Roles: {}".format(", ".join([str(role) for role in emoji.roles]))
            message += "\n"
        
        embed = Embed(title= "Server's emojis list",
                      color= COLOR,
                      description= message)
