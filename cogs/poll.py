...
class Poll(cmd.Cog):
    ...
    async def poll_create(self, ctx:cmd.Context, *, args):
        """
        **[question] | [proposition] | [proposition] | ... ** : submits a poll to precised channel.
        """
        args = [prop.strip() for prop in args.split(sep= self.sep)]

        if len(args) > 1:
            message = "\n\n".join(["{}: {}".format(self.figures[n+1], p) for n, p in enumerate(args[1:])])
    ...
    async def poll_channel(self, ctx:cmd.Context, channel):
        """
        **[#channel or channel name]** : sets the channel where to send polls.
        """
        ...
        if channel is None:
            ...
            embed = Embed(title= "Poll channel changed",
                          color= COLOR,
